...
def make_lsh_file_yelp(yelp_file, out_prefix, nchar=-1, minlen=-1, maxlen=50):
    ...
    lnum=0
    with io.open(yelp_file, encoding='utf-8', errors='ignore') as fopen:

        TESTING_LIMIT = -1
        if TESTING_LIMIT <= 0:
            yelpfile=[json.loads(strin)['text'].strip() for strin in fopen.readlines(nchar)]
        else:
            yelpfile=[json.loads(strin)['text'].strip() for strin in fopen.readlines(nchar)[:TESTING_LIMIT]]
        # yelpfile = yelpfile[:int(len(yelpfile)//10)]

        for doc in tqdm(nlp.pipe(yelpfile, batch_size=10000, n_threads=200),total=len(yelpfile)):
...
def make_lsh_file_giga(giga_file,out_prefix, minlen = 1, maxlen = 100):
    ...
    with io.open(giga_file, encoding='utf-8', errors='ignore') as fopen:
        for doc in tqdm(nlp.pipe(fopen, batch_size=10000, n_threads=200), total=line_ct):
            logging.captureWarnings(True)
            for ent in doc.ents:
                if ent.label_ is not '':
                    ent.merge(ent.root.tag_, ent.text, ent.label_)
...
def parallel_make_lsh(lem_dir, out_dir, nproc, batch_size=1000 * 100):
    ...
    for fout in tqdm(pool.imap_unordered(
            lsh_partial_wrap,
            list(zip(list(range(len(bytelen))), repeat(batch_size), repeat(lem_dir), repeat(out_dir), bytelen))),
            total=len(bytelen)):
...
import nltk  
def speed_diff(s1, s2, word_embeddings, n=3):
    def calculate_speed(s):
        ...
        _, avg_speed = get_speed(np.array(chunks))
        return avg_speed
    s1_speed = calculate_speed(s1)
    s2_speed = calculate_speed(s2)

    return s2_speed - s1_speed
...
def lsh_wrapper(args):
    ...
    print(sentences)
    jac_tr = args[3]
    word_embeddings = args[4]
    orig_gpe_sents = args[5]
    d_speed = 0.5
    tol = 0.1
    ...
    for i in range(len(setslist)):
        ...
        s1 = set(setslist[i])
        try:
            lnum = int(sentences[i].split(':')[0])
        except ValueError:
            print('This sentence has no line number!', sentences[i])
            continue
        sent1 = orig_gpe_sents[lnum].split(':')[1]
        # print('SENTNECE: ', sent1)
        tmp_set = []
        for c_sent in candidate_sents:
            ssplit = c_sent.decode('utf-8').rstrip().split(':')
            lnum = int(ssplit[0])
            ssplit = ssplit[1].split(' ')
            sent2 = orig_gpe_sents[lnum].split(':')[1]
            # print('SENTENCE 2: ', sent2)
            s2 = set(ssplit)
            if jaccard(s1, s2) > jac_tr and abs(speed_diff(sent1, sent2, word_embeddings) - d_speed) < tol:
...
def lsh_query_parallel(lsh_filenames, setslist, sentences, nproc=1, jac_tr=0.5, word_embeddings=None, orig_gpe_sents=[]):
    ...
    for query_tmp in tqdm(pool.imap_unordered(lsh_wrapper,
                                              list(zip(lsh_filenames, repeat(setslist), repeat(sentences), repeat(jac_tr), repeat(word_embeddings), repeat(orig_gpe_sents)))),
                                              smoothing=0.0, total=len(lsh_filenames)):
        for (k, v) in list(query_tmp.items()):
...
def lsh_query(lsh_filenames, setslist, sentences, jac_tr=0.5, word_embeddings=None, orig_gpe_sents=[]):
    """ Single-core version of lsh_query_parallel, see above documentation."""
    query_results = defaultdict(list)
    for filename in tqdm(lsh_filenames):
        tmp_dict = lsh_wrapper([filename, setslist, sentences, jac_tr, word_embeddings, orig_gpe_sents])
...
def grab_seeds(filename, num_lines):
    ...
    with io.open(filename, 'r', encoding='utf-8', errors='ignore') as fhandle:
        ...
        print(f'Total: {ltot}, Sample: {num_lines}')
        line_ids = np.sort(np.random.choice(ltot, size=int(num_lines), replace=False))
        lskip = line_ids[0]
        num_sampled = 0
    ...
    for sent in tqdm(queue_tmp):
        ...
        if len(nb_cands) > 1:
            nbstring = sent.rstrip() \
                       + ',' + ','.join([strsub.decode('utf-8').rstrip() for strsub in nb_cands]) + '\n'
...
def generate_adjlist(queue_input, batch_size, prefix, lsh_filename, orig_gpe_sents, n_rounds=10, nproc=1, jac_tr=0.5, word_embeddings=None):
    """
    ...
    print(f"Num Rounds: {n_rounds}")
    for batch_id in range(n_rounds):
        #
        batch_set_input = [input_sent.split(':')[1].split(' ') for input_sent in queue_tmp]
        if nproc==1:
            batch_neighbors = lsh_query(lsh_filename, batch_set_input, queue_tmp, jac_tr, word_embeddings, orig_gpe_sents)
        else:
            batch_neighbors = lsh_query_parallel(lsh_filename, batch_set_input, queue_tmp, nproc, jac_tr, word_embeddings, orig_gpe_sents)
        #
        print('generating next set')
        print(f"Len queue: {len(queue_tmp)}, {len(queue_input)}")
...
def split_adjlist(orig_gpe_input, adjlist_input, prefix_out):
    ...
    with io.open(orig_gpe_input, 'r', encoding='utf-8') as gpe_file:
        ...
        for line in tqdm(gpe_file, total=get_line_number(orig_gpe_input)):
            ...
            i += 1
        print(f'[splitadj] First loop ran: {i} times')
    with io.open(adjlist_input, 'r', encoding='utf-8') as adjlist_file:
        ...
        for line in tqdm(adjlist_file, total=get_line_number(adjlist_input)):
            ...
            i += 1
        print(f'[splitadj] Second loop ran: {i} times')
    gpe_out.close()
    numadj_out.close()
...
def dump_stop(test_dir):
    stopwords = list(nlp.Defaults.stop_words) + [p for p in string.punctuation]
    with io.open(test_dir+'/free.txt','w', encoding='utf-8') as free_file:
...
def make_test_set(test_dir, edit_input, tot_out, maxbucket = 10, split_vec = np.array([0.9,0.05,0.05]), lower_jac = 0.5):
    ...
    dump_stop(test_dir)
    with io.open(edit_input,'r',encoding='utf-8') as edit_in_file, io.open(train_output,'w',encoding='utf-8') as train_out_file, \
