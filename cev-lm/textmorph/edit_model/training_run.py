...
class EditDataSplits(object):
    ...
    def __init__(self, data_dir, use_diff):
        ...
        def examples_from_file(path):
            ...
            # total_lines = int(os.system('wc -l {}'.format(path)).split()[0])
            total_lines = int(os.system('wc -l {}'.format(path)))
...
# TODO: make this a classmethod on RandomState?
@contextmanager
...
# TODO: reduce coupling with RandomState
@contextmanager
...
class EditTrainingRun(TorchTrainingRun):
    ...
    def evaluate(self, big_eval=True):
        ...
        noiser = EditNoiser(self.config.editor.ident_pr, self.config.editor.attend_pr)
        self._evaluate(self.config, self.editor, self._examples, None, None, 0, noiser, big_eval, log=False)
    ...
    def _build_editor(cls, config):
        ...
        # file_path = join("./cev-lm/word_vectors", config.wvec_path)
        ...
        editor = Editor(source_token_embedder, config.hidden_dim, config.agenda_dim, config.edit_dim, config.lamb_reg, config.norm_eps, config.norm_max, config.kill_edit, decoder_cell, config.encoder_layers, config.perturb_val)
    ...
    def _finite_grads(cls, parameters):
        ...
        for param in parameters:
            try:
                if not np.isfinite(param.cpu().grad.data.sum()):
                    return False
    ...
    def _train(cls, config, train_state, examples, workspace, metadata, tb_logger):
        ...
        with random_state(train_state.random_state):
            ...
            while True:
                # TODO: this shuffle and the position within the shuffle is not properly restored upon reload
    ...
    def _evaluate(cls, config, editor, examples, metadata, tb_logger, train_steps, noiser, big_eval, log=True):
        ...
        def evaluate_on_examples(name, examples, save_test_outputs=False):
            ...
            if not big_eval:
                for tr in edit_traces:
                    # print(tr)
                    print('\n'.join([str(tr.example), str(tr.decoder_trace)]))

            if save_test_outputs:
                detokenizer = TreebankWordDetokenizer()

                # save to dataset path
                preds_path = join(config.dataset.path, "preds")
                if not exists(preds_path):
                    os.mkdir(preds_path)

                with open(join(preds_path, f'{config.editor.perturb_val}_test_preds.txt'), 'w+') as f:
                    for tr in edit_traces:
                        source = detokenizer.detokenize(tr.example.source_words).strip()
                        target = detokenizer.detokenize(tr.example.target_words).strip()
        ...
        evaluate_on_examples('test', examples.test, save_test_outputs=True)
    ...
    def _compute_metrics(cls, editor, examples, num_evaluate_examples, noiser,
        ...
        bleus = []
        for ex, output in zip(noised_sample, outputs):
            # outputs is a list(over batches)[ list(over beams) [ list(over tokens) [ unicode ] ] ] object.
            bleus.append(bleu(ex.target_words, output[0]))

        # cands = [ex.target_words for ex in noised_sample]
        # refs = [output[0] for output in outputs]
