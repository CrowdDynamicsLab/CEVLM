...
class AttentionDecoderCell(DecoderCell):
    def __init__(self, token_embedder, agenda_dim, decoder_dim, encoder_dim, attn_dim, no_insert_delete_attn, num_layers):
        ...
        self.vocab_projection_pos = Linear(z_dim, input_dim)  # TODO: these big params may need regularization
    ...
    def forward(self, rnn_state, decoder_cell_input, advance):
        ...
        # TODO: prevent model from putting probability on UNK
...
class AttentionTrace(UnicodeMixin):
    ...
    def __unicode__(self):
        ...
        s = ' '.join('{}[{:.2f}]'.format(t, w) for t, w in zip(self.tokens, self.attention_weights))
        return '{:10}[{:.2f}]: {}'.format(self.name, total_mass, s)
