...
class Editor(Module):
    ...
    def __init__(self, token_embedder, hidden_dim, agenda_dim, edit_dim, lamb_reg, norm_eps, norm_max, kill_edit, decoder_cell, encoder_layers, perturb):
        """Construct Editor.
        ...
        self.encoder = Encoder(token_embedder, agenda_dim, edit_dim, hidden_dim, lamb_reg, norm_eps, norm_max, kill_edit, encoder_layers,
                                   rnn_cell_factory=LSTMCell, perturb=perturb)
    ...
    def test_batch(self, examples):
        ...
        if len(examples) > 1:
            ...
            if abs(lindivid.data.cpu().numpy() - ltogether.data.cpu().numpy()) > 1e-3:
                print(examples[0:2])
                print('individually:')
                print(lindivid)
                print('batched:')
    ...
    def edit(self, examples, max_seq_length=35, beam_size=5, batch_size=256):
        ...
        beam_list = []
        edit_traces = []
        for batch in chunks(examples, int(batch_size / beam_size)):
    ...
    def _edit_batch(self, examples, max_seq_length, beam_size):
        ...
        return beams, [EditTrace(ex, d_trace.beam_traces[-1]) for ex, d_trace in zip(examples, decoder_traces)]
...
class EditTrace(UnicodeMixin):
    ...
    def __unicode__(self):
        return '\n'.join([str(self.example), str(self.decoder_trace)])
...
class EditExample(namedtuple('EditExample', ['source_words', 'insert_words', 'insert_exact_words', 'delete_words',
    ...
    def __unicode__(self):
        fmt = lambda name, delim, tokens: '{}: {}'.format(name, delim.join(tokens))
        ...
        lines = []
        lines.append(fmt('SOURCE', ' ', self.source_words))
        if insert:
            lines.append('INSERT: {}'.format(insert))
        if delete:
            lines.append('DELETE: {}'.format(delete))
        if insert_exact:
            lines.append('INSERT EXACT: {}'.format(insert_exact))
        if delete_exact:
            lines.append('DELETE EXACT: {}'.format(delete_exact))
        if self.target_words:
            lines.append(fmt('TARGET', ' ', self.target_words))
        return '\n'.join(lines)
