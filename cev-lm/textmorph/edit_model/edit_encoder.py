...
class EditEncoder(Module):
    ...
    """
    def __init__(self, word_dim, edit_dim, kappa_init, norm_eps, norm_max, perturb=False):
        super(EditEncoder, self).__init__()
        self.linear = Linear(edit_dim, edit_dim)
        self.linear_prenoise = Linear(word_dim, int(edit_dim/2), bias=False)
    ...
    def forward(self, insert_embeds, insert_embeds_exact, delete_embeds, delete_embeds_exact, draw_samples = False, draw_p = False):
        ...
        insert_set = self.linear_prenoise(insert_embed) # (batch_size, edit_dim/2)
        delete_set = self.linear_prenoise(delete_embed) # (batch_size, edit_dim/2)

        if len(insert_set.shape) < 2:
            insert_set = insert_set.unsqueeze(0)

        if len(delete_set.shape) < 2:
            delete_set = delete_set.unsqueeze(0)

        combined_map = torch.cat([insert_set, delete_set], 1) # (batch_size, edit_dim)
        if draw_samples:
            if draw_p:
    ...
    def sample_vMF(self, mu, kappa):
        ...
        for i in range(batch_size):
            ...
            if float(mu[i].norm().data.cpu().numpy()) > 1e-10:
                ...
                # ===============
                # TODO: apply normal distribution
                if self.perturb != 0.0:
                    perturb_distr = torch.normal(mean=torch.tensor([self.perturb]), std=torch.tensor([1.0])).cuda()
                    sampled_vec *= perturb_distr
                # ===============
            else:
