...
class BotoFormGenerator(object):
    ...
    def _from_data(form_data):
        ...
        def make_args(args_dict, args_):
            ...
            # keyword arguments
            kwargs = {k: v for k, v in args_dict.items() if k not in args_}
            return args, kwargs

        def add_field(obj, field):
            (fl_name, fl_value) = next(iter(field.items()))
        ...
        if k in BotoFormGenerator.form_types:
            ...
            for arg_k, arg_v in v.items():  # iterate over arguments to the form object
