...
class MTurkData(object):
    ...
    def get_complete_hits(self):
        return [h for (h, k) in self.data['hits'].items() if k['is_complete']]

    def get_incomplete_hits(self):
        return [h for (h, k) in self.data['hits'].items() if not k['is_complete']]
    ...
    def get_without_workers(self, blacklist):
        ...
        for p_id, p_data in data['pairs'].items():
    ...
    def dedupe(self):
        data = copy.deepcopy(self.data)
        for p_id, p_data in self.data['pairs'].items():
            add_workers = {}
...
class CoherenceData:
    ...
    def load(self):
        ...
        with open(self.sentences_file, 'r') as sents_f:
            ...
            line_numbers = list(range(1, len(sentences) + 1))
...
class SimilarityHIT(object):
    ...
    def save(self, database):
        for pair_id, (s_1, s_2) in self.questions.items():
...
class CoherenceHIT:
    ...
    def save(self, database):
        for sent_id, s in self.questions.items():
...
def review():
    ...
    workers_to_review = [w_id for (w_id, hits) in workers_to_hits.items(
